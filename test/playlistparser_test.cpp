...
// 1. Positive Test
TEST(PlaylistParser, playlistParserTest1) {
    ...
    create_file("testdata/SCORE/playlist1.m3u",
                ...
                "smfscore1.mid\n"
                "");
    PlaylistParser parser("testdata/SCORE/playlist1.m3u");
...
// 2. Negative Test
TEST(PlaylistParser, playlistParserTest2) {
    create_file("testdata/SCORE/playlist2.m3u",
                "textscore1txt\n"  // filetype undefined
                "\n"
                "#Smf(MIDI)\n"
                "smfscore1.failed\n"  // filetype failed
                "");
    PlaylistParser parser("testdata/SCORE/playlist2.m3u");

    EXPECT_FALSE(parser.loadScore(0));
    EXPECT_EQ(0, parser.getNumberOfScores());
    EXPECT_EQ(0, parser.getRootTick());
    EXPECT_STREQ("", parser.getTitle(0).c_str());
    EXPECT_STREQ("", parser.getFileName().c_str());

    ScoreParser::MidiMessage msg;
    EXPECT_FALSE(parser.getMidiMessage(&msg));
}
